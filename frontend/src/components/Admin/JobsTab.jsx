...
import { Trash2, ShieldCheck, RefreshCw, Calendar, Briefcase } from 'lucide-react';
...
export default function JobsTab() {
  ...
  const { jobs, jobTemplates, loadingStates } = useApp();
  ...
  const handleDelete = async (job, jobName) => {
    if (window.confirm(`Are you sure you want to delete this ${jobName} job? This will release all booked machine dates.`)) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-bold text-foreground text-left">Active Production Runs</h3>
            <p className="text-xs text-muted-foreground text-left">
              Track manufacturing job orders, delivery due dates, and schedule routing statuses
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="border-2 border-dashed border-border rounded-xl p-12 bg-card text-center flex flex-col items-center justify-center gap-3">
            ...
            <div>
              <h4 className="font-semibold text-foreground">No Jobs Scheduled</h4>
              <p className="text-xs text-muted-foreground mt-1">
                Enter template parameters in the scheduler form on the right to start production planning.
              </p>
            </div>
          </div>
        ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-secondary/40 border-b border-border">
                    <th className="py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Job ID</th>
                    <th className="py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Job Name</th>
                    <th className="py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Quantity</th>
                    <th className="py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Due Date</th>
                    <th className="py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Completion</th>
                ...
                <tbody className="divide-y divide-border">
                  {jobs.map((job) => {
                    ...
                    return (
                      <tr key={job.id} className="hover:bg-secondary/20 transition-colors duration-150">
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-2 font-bold text-foreground">
                            ...
                            <span className="capitalize">{jobName}</span>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-6">
                          {isCompleted ? (
                            ...
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-950/25 dark:text-blue-400 dark:border-blue-900/30">
                              ...
                              🔄 {Math.round(job.completion_percentage)}% In Progress
                            </span>
                          )}
                        </td>
